...
if [ "$MASTERFLAG" =  false ]; then
    ...
    docker-machine create --driver virtualbox --virtualbox-memory $MANAGER_MEMORY $MANAGER_NAME
fi
...
do 
    ...
    docker-machine create --driver virtualbox --virtualbox-memory $WORKER_MEMORY "$WORKER_NAME-${WORKERS}"
done